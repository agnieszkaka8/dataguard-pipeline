...
|---|---|---|---|---|---|---|
| 1 | Quality-gates wiring | Wire pytest+mypy+ruff into CI on every push/PR — today's only workflow runs the production sync job, never a test | cross-cutting | gates | change opened | `context/changes/testing-quality-gates-wiring/` |
| 2 | Critical-path regression guard | Lock the write-order guarantee and the no-leakage hard rule with dedicated tests that fail loudly if violated | #2, #3 | unit + contract | not started | — |
